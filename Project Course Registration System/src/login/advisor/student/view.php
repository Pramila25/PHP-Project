...
echo '<h1 class="display-4 mx-auto text-center mt-4">Student Information</h1>';
...
while($row = $result->fetch_array(MYSQLI_ASSOC)){   //Creates a loop to loop through result
    ...
    echo "<form  method='POST' action='student_detail.php'>"."<tr><td>" . $row['uid'] . "</td><td>" . $row['fname'] . "</td><td>". $row['lname'] . "</td><td>". $row['dob'] .
        "</td><td>". $row['address'] . "</td><td>". $row['contact_no'] . "</td><td>". $row['program_id'] . "</td><td>". "<input type='hidden' name='uid'
    value='$row[uid]'>"."</td><td>"."<input type='submit' value='View Detail'>"."</td></tr>"."</form>";
