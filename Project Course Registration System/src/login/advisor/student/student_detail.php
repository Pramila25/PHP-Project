...
$query = "SELECT * FROM studentinfo WHERE uid = '$passedUID' ";
...
while($row = $result->fetch_array(MYSQLI_ASSOC)){   //Creates a loop to loop through result
    ...
    echo "<tr><td>" . $row['uid'] . "</td><td>" . $row['fname'] . "</td><td>". $row['lname'] . "</td><td>". $row['dob'] .
        "</td><td>". $row['address'] . "</td><td>". $row['contact_no'] ."</td></tr>";
}
...
<form action="delete.php"  method="post" class="d-flex justify-content-center">
    ...
    <input type='hidden' name='uid' value='$passedUID'>
